...
def bollinger_bands(codes, timeperiod=20, nbdevup=2, nbdevdn=2, enddayshift=0):
    '''
            多股票布林带
    依赖库：
        import talib as tl
    ...
    df = history(timeperiod*2+enddayshift, '1d', 'close' , codes, skip_paused=True)[:timeperiod*2]
    def func(xs): 
...
def predict_stock_trend(context, stock, stock_in_strong_tone = False, stock_in_risk = False, stock_sell_signal = False, days_predict = 60, log_bar_off = False):
    def trend_calculate(stock, stockname, date, upperlimit, lowerlimit, middleupperlimit, middlelowerlimit, stock_in_strong_tone, stock_in_risk, stock_sell_signal):
        ...
        # 计算指数强弱
        if stock_in_strong_tone and stock_price < middlelowerlimit:
            stock_in_strong_tone = False
        elif not stock_in_strong_tone and stock_price > middleupperlimit:
        ...
        # 或强势区域进入布林线上轨上方
        # 清仓
        if not stock_in_risk and stock_last_status:
        ...
        # 或弱势区域进入布林线下轨下方
        # 建仓
        elif stock_in_risk and not stock_last_status:
    ...
    for i in range(days_predict-1):
        ...
        while date_tmp not in trading_days:
            date_tmp = date_tmp + timedelta(days=-1)
    date_index = trading_days.index(date_tmp)
    # 计算对比标杆指数布林线参数
    for i in range(days_predict-1, -1, -1):
        df = bollinger_bands([stock], enddayshift=i)
        # 布林线上轨上沿阈值
        ...
        if i != 0:
            stock_price = get_close_price(stock, i, '1d')
        else:
